...
var common_splitPage = function() {
	...
	var splitPageHtml = function(divId, formId, totalRow, pageSize, pageNumber, totalPages, isSelectPage, isSelectSize, orderColunm, orderMode, currentPageCount){
		...
		splitStr += '<span style="float:left">显示<strong>' + start + '至<strong>' + end + '</strong>条，共<strong>' + totalRow + '</strong>' + i18n_common_splitPage_records + '</span>';
		splitStr += '<span class="dataTables_paginate paging_bootstrap" style="float: right;">';
		...
		if (pageNumber == 1 || totalPages == 0) {
			...
			splitStr += '<li class="prev"><a href="javascript:common_splitPage.splitPageLink(\''+divId+'\', \''+formId+'\', ' + (pageNumber - 1) + ');">' + i18n_common_splitPage_previous + '</a></li>';
		}
		...
		for (var i = 1; i <= totalPages; i++) {
	        if (i == 2 && pageNumber - 4 > 1) {
	            ...
	            if (pageNumber == i) {
	            	splitStr += '<li class="paginate_button active" style="background-color:#5d9cec;color:#5d9cec"><a href="javascript:void(0)">' + pageNumber + '</a></li>';
	            } else {
		...
		if (pageNumber == totalPages || totalPages == 0) {
			...
			splitStr += '<li class="next"><a href="javascript:common_splitPage.splitPageLink(\''+divId+'\', \''+formId+'\', ' + (pageNumber + 1) + ');">' + i18n_common_splitPage_next + '</a></li>';
		}
		...
		//splitStr += '&nbsp;&nbsp;<li style="float:left">显示<strong>' + start + '至<strong>' + end + '</strong>条，共<strong>' + totalRow + '</strong>' + i18n_common_splitPage_records + '</li>';
